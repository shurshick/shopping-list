...
© 2026 Александр Коваленко

Контакт: `shurshick@bk.ru`
...
- экран конфликтов, если один товар был изменён на двух телефонах до синхронизации;
- внутренняя архитектура backend и Android разделена на более понятные слои без изменения UX и API;
- короткий статус синхронизации без технических деталей;
...
- опциональное объединение одинаковых товаров при добавлении с увеличением количества; настройка находится в меню списка под кнопкой-шестерёнкой;
...
- проверка обновлений Android-приложения через GitHub Releases при запуске: если доступна новая APK-версия, её можно скачать и сразу передать в установщик Android;
...
- тёмная Material-подобная админ-панель с плиточной навигацией, поиском, фильтрами, адаптивными таблицами и безопасными действиями над пользователями, списками и приглашениями;
