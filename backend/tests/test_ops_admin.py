...
def test_sync_records_client_version_for_admin_users(client):
    ...
    sync_response = client.get(
        ...
        headers={
            **auth(admin_token),
            ...
            "X-Client-Version": "1.4.6",
            "X-Client-Version-Code": "29",
    ...
    assert user["last_client_version"] == "1.4.6"
    assert user["last_client_version_code"] == 29
...
def test_metrics_contains_counts_and_no_email(client):
    ...
    assert response.status_code == 200
    assert response.json()["version"] == "1.4.6"
