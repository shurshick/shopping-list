...
- готовый образ API из GitHub Container Registry.
...
Для обновления сервера в TrueNAS замените YAML на актуальный из проекта и убедитесь, что в секции `api` указан новый образ:

```yaml
image: ghcr.io/shurshick/shopping-list-api:v1.2.1
```

После этого остановите и снова запустите приложение. Если версия в `/docs` не изменилась, удалите старый контейнер приложения в TrueNAS и создайте его заново с тем же volume PostgreSQL. Данные списков находятся в volume `postgres_data`, поэтому сам контейнер API можно пересоздавать.
