...
- `/admin/users` - пользователи, поиск по email, фильтры по статусу, сортировка, блокировка, разблокировка, смена пароля.
- `/admin/lists` - все списки, поиск по названию, фильтры active/archive, детали, archive/restore.
- `/admin/invites` - invite-ссылки, фильтры active/used/expired/revoked и отзыв активных приглашений.
- `/admin/system` - версия, БД, миграции, uptime, режим регистрации.
- `/admin/logs` - последние события приложения, фильтры и копирование отчёта.
- `/admin/diagnostics` - сводка для диагностики и копирование безопасного отчёта.

Веб-страница `/admin` имеет общую навигацию по разделам. На узких экранах таблицы прокручиваются по горизонтали, а действия переносятся без поломки строк.
...
- `APP_VERSION` - tag Docker image, например `latest` или `v1.5.1`.
