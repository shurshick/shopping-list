...
`GET /admin/users` поддерживает параметры:

- `query` - поиск по email;
- `status` - `all`, `active`, `disabled`, `admins`;
- `sort` - `created_at`, `email`, `last_sync`, `android_version`.
...
`GET /admin/lists` поддерживает параметры:

- `query` - поиск по названию списка;
...
`GET /admin/invites` поддерживает параметры:

- `query` - поиск по названию списка или ID списка;
- `status` - `active`, `used`, `expired`, `revoked`, `all`.
