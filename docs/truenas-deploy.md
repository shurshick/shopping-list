...
При первом запуске мастер открыт без пароля. В нем можно задать:

- внешний HTTPS-адрес сервера;
- название приложения;
- разрешена ли регистрация новых пользователей.

После сохранения мастер считается завершенным и закрывается от повторных изменений.

Для повторной настройки через веб-интерфейс добавьте переменную `SETUP_TOKEN` для контейнера `api`, перезапустите контейнер и откройте:
