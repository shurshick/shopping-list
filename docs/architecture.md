...
- `data/repository/DiagnosticsRepository.kt` выполняет безопасную проверку `/health/live`, `/health/ready` и `/server-config`.
- `domain/model/DiagnosticsInfo.kt` формирует безопасный диагностический отчёт без token, пароля, email, товаров и содержимого offline queue.
...
Экран диагностики в настройках Android показывает количество ожидающих операций, последнюю успешную синхронизацию, последнюю попытку и последнюю ошибку. Это не меняет логику offline queue и не отправляет операции отдельным путём: кнопка `Повторить синхронизацию` вызывает обычный sync flow.
